...
def define_specificity_categories(output_dir, regulatory_regions_dir):
    ...
    # expected_categories = [
    #     "fetal_specific", "stem_cell_specific", "fetal_stem_shared_specific",
    #     "developmental_specific", "adult_specific", "fetal_stem_shared"
    # ]
    ...
    try:
        ...
        # First, create a mapping of coordinates to full records from original cfPeak file
        logger.info("Step 3/5: Creating coordinate mapping from original cfPeak file...")
        coord_to_record = {}
        with open(Path(output_dir) / "original_cfpeaks_with_id.bed", 'r') as f:
            for line in f:
        ...
        categories = {
            ...
            #"fetal_stem_shared_specific": fetal_bed.intersect(stem_bed).intersect(adult_bed, v=True),
            ...
            #"fetal_stem_shared": fetal_bed.intersect(stem_bed)
        }
...
def process_cqtl_file(cqtl_file, pval_threshold=0.05, genome_build='hg19'):
...
def process_custom_random_bg(custom_bg_file, base_dir=None, target_count=None, seed=50, pval_threshold=None, mode='bed', perform_permutation=False, n_permutations=100, return_full_bedtool=False):
    ...
    try:
        ...
        if mode == 'cqtl':
            ...
            # If p-value threshold is provided, filter by significance
            if pval_threshold is not None:
                if has_header and 'pval' in df.columns:
                    original_count = len(df)
                    df = df[df['pval'] <= pval_threshold]
                    filtered_count = len(df)
                    logger.info(f"Filtered cQTLs by p-value threshold {pval_threshold}:")
        ...
        logger.info("Loaded custom random background:")
...
def save_full_random_fisher_stats(input_bed, full_random_bed, custom_regions_bed, output_dir, category_name=None):
    """Save Fisher's test results using the full set of random cfPeaks as reference."""
    from scipy import stats
    import numpy as np
    import pandas as pd
    from collections import OrderedDict
    ...
    # Save overlapping peaks files for full random background analysis
    output_dir = Path(output_dir)
    overlap_results_dir = output_dir / "overlap_results"
    overlap_results_dir.mkdir(exist_ok=True)
    
    # Save input overlapping peaks
    input_overlaps = input_bed.intersect(custom_regions_bed, u=True)
    input_overlaps_file = overlap_results_dir / f"{category_name}_overlapping_cfpeaks_full_random.bed"
    input_overlaps.saveas(str(input_overlaps_file))
    logger.info(f"Saved {len(input_overlaps)} input overlapping peaks (full random analysis) to: {input_overlaps_file}")
    
    # Save full random overlapping peaks
    full_random_overlaps = full_random_bed.intersect(custom_regions_bed, u=True)
    full_random_overlaps_file = overlap_results_dir / f"{category_name}_overlapping_full_random_bg.bed"
    full_random_overlaps.saveas(str(full_random_overlaps_file))
    logger.info(f"Saved {len(full_random_overlaps)} full random background overlapping peaks to: {full_random_overlaps_file}")
...
def perform_overlap_analysis(cfpeak_cqtl_bed, custom_regions_bed, custom_random_bg, output_dir, category_name=None, mode='bed', source_files=None, perform_permutation=False, n_permutations=100):
    ...
    try:
        ...
        # Save overlapping peaks to file
        output_dir = Path(output_dir)
        overlap_results_dir = output_dir / "overlap_results"
        overlap_results_dir.mkdir(exist_ok=True)
        
        # Save overlapping peaks file
        overlapping_peaks_file = overlap_results_dir / f"{category_name}_overlapping_cfpeaks.bed"
        overlaps.saveas(str(overlapping_peaks_file))
        logger.info(f"Saved {len(overlaps)} overlapping peaks to: {overlapping_peaks_file}")
        
        # Also save overlapping peaks with full record information (including identifiers)
        overlapping_peaks_with_id_file = overlap_results_dir / f"{category_name}_overlapping_cfpeaks_with_id.bed"
        overlapping_regions = cfpeak_cqtl_bed.intersect(custom_regions_bed, u=True)
        overlapping_regions.saveas(str(overlapping_peaks_with_id_file))
        logger.info(f"Saved overlapping peaks with identifiers to: {overlapping_peaks_with_id_file}")
        ...
        if perform_permutation and random_bg_metadata.get('perform_permutation', False):
            ...
            mean_permutation_odds = np.mean(permutation_odds_ratios)
            std_permutation_odds = np.std(permutation_odds_ratios)
            ...
            # # 1. Bar plot comparing observed vs expected
            # fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
            
            # # Fold enrichment bar plot
            # ax1.bar(['Observed', 'Expected'], 
            #        [fold_enrichment, mean_permutation_fold],
            #        yerr=[0, std_permutation_fold],
            #        capsize=10,
            #        color=['#2ecc71', '#3498db'])
            # ax1.set_title('Fold Enrichment Comparison')
            # ax1.set_ylabel('Fold Enrichment')
            # ax1.text(0, fold_enrichment, f'{fold_enrichment:.2f}', 
            #         ha='center', va='bottom')
            # ax1.text(1, mean_permutation_fold, f'{mean_permutation_fold:.2f}', 
            #         ha='center', va='bottom')
            
            # # Odds ratio bar plot
            # ax2.bar(['Observed', 'Expected'], 
            #        [odds_ratio, mean_permutation_odds],
            #        yerr=[0, std_permutation_odds],
            #        capsize=10,
            #        color=['#2ecc71', '#3498db'])
            # ax2.set_title('Odds Ratio Comparison')
            # ax2.set_ylabel('Odds Ratio')
            # ax2.text(0, odds_ratio, f'{odds_ratio:.2f}', 
            #         ha='center', va='bottom')
            # ax2.text(1, mean_permutation_odds, f'{mean_permutation_odds:.2f}', 
            #         ha='center', va='bottom')
            
            # plt.tight_layout()
            # plt.savefig(plots_dir / f"{category_name}_comparison_plot.pdf", dpi=300, bbox_inches='tight')
            # plt.close()
            ...
            ax2.hist(permutation_odds_ratios, bins=20, alpha=0.7, color='#3498db')
            ...
            permutation_results = {
                ...
                'all_p_values': permutation_p_values,  # Add p-values to results
        ...
        # --- Save full random background Fisher's test ---
...
def create_detailed_summary(results_dict, output_dir, category_name, source_files=None):
    ...
    with open(summary_file, 'a') as f:
        ...
        # Write saved files information
        f.write("Saved Overlapping Peaks Files:\n")
        f.write("-" * 50 + "\n")
        f.write(f"Overlapping peaks file: overlap_results/{category_name}_overlapping_cfpeaks.bed\n")
        f.write(f"Overlapping peaks with IDs: overlap_results/{category_name}_overlapping_cfpeaks_with_id.bed\n")
        f.write(f"Full random analysis overlapping peaks: overlap_results/{category_name}_overlapping_cfpeaks_full_random.bed\n")
        f.write(f"Full random background overlapping peaks: overlap_results/{category_name}_overlapping_full_random_bg.bed\n\n")
...
def sanity_check():
    ...
    try:
        ...
        test_paths['base_dir'] = os.path.join(test_paths['base_dir'], test_paths["regulatory_state"])
        ...
        # 2. Test process_cfpeak_file
        print("\nTesting process_cfpeak_file()...")
        cfpeak_bed, total_cfpeaks = process_cfpeak_file(
            test_paths['cfpeak_file'],
            test_paths['base_dir']
        )
        print(f"✓ Processed {total_cfpeaks} cfPeaks")
        ...
        for category_name, bed_file in category_files.items():
            ...
            source_files = {
                'input_file': test_paths['cfpeak_file'],
...
# Save the original input bed to base_dir
input_bed_file = os.path.join(test_paths['base_dir'], "original_cfpeaks_with_id.bed")
cfpeak_bed.saveas(input_bed_file)
print(f"✓ Saved original input bed to: {input_bed_file}")
...
# Save the full random background bed to base_dir
full_random_bed_file = os.path.join(test_paths['base_dir'], "full_random_bg_with_id.bed")
full_random_bed.saveas(full_random_bed_file)
print(f"✓ Saved full random background bed to: {full_random_bed_file}")

# # Save unique input bed regions not overlapping with full random background
unique_input_not_in_random = cfpeak_bed.intersect(full_random_bed, v=True)
unique_outfile = os.path.join(test_paths['base_dir'], "cfpeaks_unique_to_WBCPeaks.bed")
unique_input_not_in_random.saveas(unique_outfile)
print(f"✓ Saved unique input cfPeaks not in full random background to: {unique_outfile}")

# Remove random_background directory if it exists
import shutil

random_bg_dir = os.path.join(test_paths['base_dir'], "random_background")
if os.path.exists(random_bg_dir):
    shutil.rmtree(random_bg_dir)
    print(f"✓ Removed random_background directory: {random_bg_dir}")
else:
