...
cancer.list <- c("PASS_OvarianCancer","ProstateCancer_Meta_Schumacher2018.nodup",
...
noncancer.list <- c("PASS_Alzheimers_Jansen2019","PASS_Crohns_Disease","PASS_IBD_deLange2017",
                    "PASS_Rheumatoid_Arthritis","PASS_Schizophrenia",
...
cancer.df    <- summarise_set(cancer.list)
noncancer.df <- summarise_set(noncancer.list)

# --- Nice labels
pretty_trait <- function(x) dplyr::case_when(
  x == "PASS_OvarianCancer" ~ "Ovarian Cancer",
  x == "ProstateCancer_Meta_Schumacher2018.nodup" ~ "Prostate Cancer",
  x == "UKB_460K.cancer_MELANOMA" ~ "Melanoma",
  x == "breast_cancer" ~ "Breast Cancer",
  x == "ec" ~ "Endometrial Cancer",
  x == "lc" ~ "Lung Cancer",
  x == "rcc_meta" ~ "Renal Cell Carcinoma",
  x == "PASS_Alzheimers_Jansen2019" ~ "Alzheimer's Disease",
  x == "PASS_Crohns_Disease" ~ "Crohn's Disease",
  x == "PASS_IBD_deLange2017" ~ "Inflammatory Bowel Disease",
  x == "PASS_Rheumatoid_Arthritis" ~ "Rheumatoid Arthritis",
  x == "PASS_Schizophrenia" ~ "Schizophrenia",
  x == "PASS_Ulcerative_Colitis" ~ "Ulcerative Colitis",
  #x == "UKB_460K.disease_PSORIASIS" ~ "Psoriasis",
  TRUE ~ x
)
...
pretty_panel <- function(x) dplyr::case_when(
  ...
  x == "cfChIP_H3K4me3" ~ "H3K4me3 cfChIP",
  TRUE ~ x
)
...
# --- Build and save
p_cancer    <- make_bar(cancer.df,    "Manuscripts/Figures/4_g.pdf")
p_noncancer <- make_bar(noncancer.df, "Manuscripts/Figures/4_h.pdf")

write.csv(cancer.df, "Results/cancer.df.csv")
write.csv(noncancer.df, "Results/noncancer.df.csv")



########Pair-end t-test
cancer.df = cancer.df %>%
  filter(panel %in% c("whole-blood", "cfChIP_H3K27ac"))


tab_list <- cancer.df %>%
  mutate(no.nonsig = no.cv - no.sig) %>%
  select(trait, panel, no.sig, no.nonsig) %>%
  group_split(trait)


make_mat <- function(d){
  m <- as.matrix(d[match(unique(d$panel), d$panel), c("no.sig","no.nonsig")])
  rownames(m) <- unique(d$panel)
  colnames(m) <- c("sig","nonsig")
  m
}
mats <- lapply(tab_list, make_mat)
arr  <- simplify2array(mats)   # dims: 2 x 2 x K
mantelhaen.test(arr)           # CMH test (stratified by trait)


noncancer.df = noncancer.df %>%
  filter(panel %in% c("whole-blood", "cfChIP_H3K27ac"))


tab_list <- noncancer.df %>%
  mutate(no.nonsig = no.cv - no.sig) %>%
  select(trait, panel, no.sig, no.nonsig) %>%
  group_split(trait)
